    ...
    $.fn, {
        ...
        bitmapCutter: function (settings) {
            ...
            var ps = $.fn.extend({
                ...
                holderSize: { width: 300, height: 300 },
            ...
            $img.loadBitmap(function (e) {
                ...
                $generate.click(function () {
                    ...
                    if (typeof ps.onUpload == "function") {
                        ps.onUpload();
                        return false;
                    }

                    $.ajaxFileUpload({
                        url: 'upload-class.php',//用于文件上传的服务器端请求地址
                        secureuri: false,//
                        data: {
                            'x1': Math.abs($thumbimg.f('left')), 'y1': Math.abs($thumbimg.f('top')), 'zoom': $bcglobal.$zoomValue,
                        ...
                        success: function (data, status)  //服务器成功响应处理函数
                        {
                            ...
                            if (data.isSuccess == "1") {
                                ps.onGenerated(data.url);
                                alert(ps.holderSize.width);
                            } else {
                                alert(data.message);
                            }
                        },
