...
$(function () {
    ...
    SetWechatShare("羽西", "羽西", location.href.split("?")[0], "images/bg/land_bg.jpg");
...
(SiteFunc = new function () {
    ...
    //分享
    this.Share = function (url, shareImg) {
        var _url = url;
        var _shareImg = shareImg || "images/bg/land_bg.jpg";
        ...
        if (Cmn.Func.IsWeiXin()) {
            ...
            SetWechatShare("羽西", "羽西", _url, _shareImg, function () {
    ...
    this.SetBottomNav = function (ReturnUrl) {
        var _returnUrl = ReturnUrl || location.href.indexOf("returnurl") >= 0 ? location.href.substring(0, location.href.indexOf("returnurl") - 1) : location.href.toString();
        $(".footer-nav a").each(function () {
            var _href = $(this).attr("href").toString().split("?")[0];
            if (_returnUrl.split("?")[0].indexOf(_href) < 0) {
