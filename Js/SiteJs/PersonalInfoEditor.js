...
$(function () {
    ...
    function BindUpload1(FileInput, ShowBox) {
        window._Stage1 = Cmn.UI.CanvasTools.ImageProcess(ShowBox, 640, 490);
        _Stage1.BindSelectFileBtn(FileInput, 5);
        _Stage1.OnSelectFile.Add(function (image) {
            //浮层显示出来
    ...
    $(".JscSubmitPicture").on("click", function () {
        ...
        var _imageData;
        if (!_Switching) {
            _imageData = window._Stage1.GetBase64Image();
        }
        else {
            _imageData = window._Stage.GetBase64Image();
        }
        var _param = {
