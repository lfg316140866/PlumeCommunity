...
$(document).ready(function () {
    ...
    $(".concern-btn").bind("click", function () {
        ...
        _workListFollow.BindScrollLoadData(".Inner2", 300);
    ...
    $(".topics-btn").bind("click", function () {
        ...
        _workListLabel.BindScrollLoadData(".Inner1", 300);
    });
    ...
    $(".Inner").on("click", ".ShareBtn", function() {
        SiteFunc.Share(window.location.href.split("?")[0] + "?WorkID=" + $(this).attr("workid"), $(this).parents(".con-item").find(".con-des>img").eq(0).attr("src"));
    });
    ...
    $(".DescMain").on("click", ".BtnToUser", function (event) {
        event.stopPropagation();
        SiteFunc.JumpPage("Personal.aspx" + ($(this).parent().attr("userid") == "self" ? "" : "?uid="+$(this).parent().attr("userid")), "Home.aspx?workid=" + $(this).parents(".con-box").attr("workid"));
    });
    ...
    _workListHot.BindScrollLoadData(".Inner1",300);
    if (_NoLogin) {
        return false;
    }
...
var YQFunc = {
        ...
        Cmn.Ajax.PostData("/Itf/CSharp/Interface.aspx?method=GetFollowPraiseCollectState", _param, function (Statedata) {
            if (Statedata.IsSuccess == 1) {
                ...
                if (Statedata.data[0]["ReportID"] != "") {
                    Container.find(".report a").attr("reportid", Statedata.data[0]["ReportID"]).css("visibility", "hidden");
                } else {
                    Container.find(".report a").attr("reportid", Statedata.data[0]["ReportID"]).css("visibility", "visible");
                }
                ...
                Container.find(".DescState").SgVisible();
    ...
    DescCommentBtnBind: function () {
        $(".DescMain .CommentBtn").on("click", function () {
            //setTimeout(function () {
            //$("#Comment_Input").focus();
            var editableDiv = document.getElementById("Comment_Input");
    ...
    GetWorkImgAndLabelByWorkID: function (WorkID) {
        ...
        CmnAjax.PostData("/Itf/CSharp/Interface.aspx?method=GetWorkImgAndLabelByWorkID", _param, function (data) {
            if (data.IsSuccess == 1) {
                if (data.data == "") {
                    alert("该图片已被删除");
                    $(".MainList .con-box[workid=" + WorkID + "]").remove();
