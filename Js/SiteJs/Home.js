...
$(document).ready(function () {
    ...
    $(".DescMain").on("click", ".list-desc", function (event) {
        ...
        $("#Comment_Input").attr({ "placeholder": "回复" + _self.find(".comment-name span").html(), "replyuserid": _self.attr("userid"), "replyusername": _self.find(".comment-name span").html() });
    });


    $(".DescMain .Inner").click(function () {
        $("#Comment_Input").attr({ "placeholder": "我也说一句", "replyuserid": "", "replyusername": "" });
    });
...
var YQFunc = {
        ...
        Cmn.Ajax.PostData("/Itf/CSharp/Interface.aspx?method=GetCommentTop3ByWorkID", { WorkID: WorkID }, function (data1) {
            if (data1.IsSuccess == 1) {
                ...
                for (var _j = 0; _j < data1.data.length; _j++) {
                    ...
                    //'</div>';
                    _listHtml += YQFunc.GetCommentHtml(data1.data[_j]);
    ...
    GetCommentHtml: function (data) {
        return '<div class="list-desc" userid="' + data["UserID"] + '" commentid="' + data["CommentID"] + '">' +
                                   ...
                                   '<em class="comment-name general-font center-vertical"><span>' + data["NickName"] + '</span>' + (data["ReplyUserName"] == "" || data["ReplyUserName"] == undefined ? "" : "回复" + data["ReplyUserName"]) + ':</em>' +
                    '<strong class="comment-des general-font center-vertical">' + data["Comment"] + '</strong>' +
                    '</div>';
    },
