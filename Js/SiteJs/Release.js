...
$(function () {
    ...
    function StickersThrows() {
        CmnAjax.PostData("/Itf/CSharp/CmnMisItf.aspx?method=GetSqlData&ItfName=GetStickerCategory", "", function (dat) {
                if (dat.IsSuccess == 1) {
                    Cmn.FillData(".JscStickersElements", dat.data, false);
                    //选取贴纸分类
                    var _StickerCategoryID;
                    $(".JscStickersElements").on("click", function () {
                        var _indexf = $(this).index();
                        $(".JscStickersElements a").removeClass("select");
                        $(".JscStickersElements a").eq(_indexf).addClass("select");
                        _StickerCategoryID = $(this).attr("sid");
                        //////////////选取贴纸//////////////
                        CmnAjax.PostData("/Itf/CSharp/CmnMisItf.aspx?method=GetSqlData&ItfName=GetSticker", { "StickerCategoryID": _StickerCategoryID }, function (dat) {
                            Cmn.FillData(".JscStickersElementsSon", dat.data, false);
                            if (dat.IsSuccess == 1) {
                                $(".JscStickersShow").show();
    ...
    function PopularTagsThrown() {
        CmnAjax.FillData(".JscPopularLable", "Itf/CSharp/CmnMisItf.aspx?method=GetSqlData&ItfName=YQGetLable", "", function (dat) {
            ...
            $(".JscPopularLable").on("click", function () {
                ...
                $(".JscPhotoCanvas").append("<div class='set-label JscLableShow'><div class='choose-desc JscLeftLable JscThisLable' style='visibility: hidden;'><div class='choose-desc-box'><span class='attention-desc-text JscLableShowText'>" + _lableTextPopular + "</span><b class='attention-arrow'></b></div></div><div class='choose-desc-round'><i class='round'></i></div><div class='choose-desc tags-set JscRightLable JscThisLable' style='visibility: visible;'><div class='choose-desc-box choose-desc-fr'><span class='attention-desc-text JscLableShowText'>" + _lableTextPopular + "</span><b class='attention-arrow'></b></div></div></div>");
                ReversalLable();
            })
        }, "", "", function (data) {
            for (var _i = 0; _i < data.length; _i++) {
    ...
    function ReversalLable() {
        //标签位置切换
        touch.on(".JscThisLable", "tap", function () {
            //为右 换左
            if (_LableDirection == 0) {
                $(".JscRightLable").css("visibility", "visible");
                $(".JscLeftLable").css("visibility", "hidden");
                _LableDirection = 1;
            }
                //为左 换右
            else {
                $(".JscLeftLable").css("visibility", "visible");
                $(".JscRightLable").css("visibility", "hidden");
                _LableDirection = 0;
            }
        })
    }
    ...
    $(".JscDetermineBtn").on("click", function () {
        //把所有标签添加成JOSN
        SiteFunc.FloatOperating(".JscLayer,.JscUploadLayer", ".JscLayerHide");
        var _objectLable = "";
        var _josnLable = "";
    ...
    Cmn.UI.Drag(".JscLableShow", {
        ...
        onMove: function () {
            ...
            //var _widthLable = $(this).width();
            //var _difference = _widthWindow - _widthLable;
            //var _leftLables=$(this).css("left");
            //_leftLables = parseInt(_leftLables.replace("px", ""));
            ////当标签位置为右边时
            //if ($(".JscLableShow").find(".JscRightLable").css("visibility") == "visible") {
            //    if (_leftLables+5 > _difference) {
            //        $(this).find(".JscRightLable").css("visibility", "hidden");
            //        $(this).find(".JscLeftLable").css("visibility", "visible");
            //    }
            //}
            //else {
            //    if (_leftLables < 5) {
            //        $(this).find(".JscLeftLable").css("visibility", "hidden");
            //        $(this).find(".JscRightLable").css("visibility", "visible");
            //    }
            //}
