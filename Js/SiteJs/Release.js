...
$(function () {
    ...
    function StickersTC(ImgUrl, _modeUrlIndex) {
        ...
        SiteFunc.FloatOperating(".JscOperatingPiece", ".JscStickersShow,.JscSixPhoto,.JscFourPhoto");
    }
    ...
    function AttentionThrown(Search) {
        ...
        CmnAjax.FillData(".JscPersonalInformation", "Itf/CSharp/ItfOther.aspx?method=FollowThrownFriends", _param, function (dat) {
            ...
            $(".JscPersonalInformation").on("click", function () {
                _uid = $(".JscPersonalInformation").attr("uid");
                if ($(".JscPersonalInformation").eq($(this).index()).find(".JscATAttention").hasClass("select")) {
                    ...
                    if (_uid!="")
                    $(".JscPhotoCanvas").find(".JscLableShow[uid = " + _uid + "]").remove();
                    $(".JscChoice").hide();
                    SiteFunc.SceneJump("choice", 3);
                    ...
                    var _lableName = $(this).find(".JscUserName").html();
                    _lableName = "@" + _lableName;
                    $(".JscPhotoCanvas").append("<div uid=" + _uid + " class='set-label JscLableShow'><div class='choose-desc JscLeftLable JscThisLable' style='display:none;'><div class='choose-desc-box'><span class='attention-desc-text JscLableShowText'>" + _lableName + "</span><b class='attention-arrow'></b></div></div><div class='choose-desc-round'><i class='round'></i></div><div class='choose-desc tags-set JscRightLable JscThisLable' style='visibility: visible;'><div class='choose-desc-box choose-desc-fr'><span class='attention-desc-text JscLableShowText'>" + _lableName + "</span><b class='attention-arrow'></b></div></div></div>");
                    SiteFunc.SceneJump("choice", 3);
                }
                SiteFunc.FloatOperating(".JscHeadColumn,.JscLableDiv", ".JscOperatingHide");
                $(".JscMaterialSelection a").removeClass("select");
                $(".JscMaterialSelection a").eq(3).addClass("select");
    ...
    function Share(_UserUploadImg, _workeID) {
        ...
        if (Cmn.Func.IsWeiXin()) {
            ...
            SetWechatShare("羽西", "羽西", " Home.aspx?WorkID=" + _workeID, _UserUploadImg, function () {
    ...
    $(".JscDetermineBtn").on("click", function () {
        ...
        CmnAjax.PostData("Itf/CSharp/Upload.aspx?method=SaveFile", {
            ...
            ImageData: _ImageEdit.GetImageToBase64()
        }, function (dat) {
            ...
            CmnAjax.PostData("/Itf/CSharp/ItfOther.aspx?method=UserUploadWorks", _param, function (dat) {
                if (dat.IsSuccess == 1) {
                    var _workeID = dat.WorkID;
                    //上传完成
                    SiteFunc.FloatOperating(".JscHeadColumn,.JscUpdateLayer", ".JscUploadLayer,.JscOperatingPiece,.JscLableFloat,.JscLableShowFloat");
                    setTimeout(function () { $(".JscLableIsShow").hide(); }, 1500);
                    Share(_UserUploadImg, _workeID);
                    $(".JscMaterialSelection a").removeClass("select");
                    $(".JscMaterialSelection a").eq(0).addClass("select");
    ...
    $(".JscChoice").on("click", function () {
        var _uid = $(".JscLableShow").eq(_LobaleIndex).attr("uid");
        var _index = $(".JscPersonalInformation[uid=" + _uid + "]").index();
        if (_index != -1) {
            $(".JscPersonalInformation").eq(_index).find(".JscATAttention").removeClass("select");
        }
