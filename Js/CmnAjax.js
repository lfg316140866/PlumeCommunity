...
/*!
 * Cmn JavaScript Library version v2.1
 * http://www.cagoe.com/
 * 本框架隶属于上海市加谷网络科技有限公司
 * Date: 2015-7-11 21:15
 */

CmnAjax_Version = "2.1.2";
...
var CmnAjax =Cmn.Ajax = {
        ...
        **********************************************************/
        ...
        this.BindScrollLoadData = function (containerSelector, sensitivity) {
            ...
            function _scrollHeader() {
                ...
                if (_$scrollContainer.scrollTop() + _$scrollContainer.height() + _sensitivity >= _scrollHeight) {
    ...
    Func: {
        ...
        _ItfToken:"",
        SetItfToken: function (token) {
            /// <summary>设置接口Token</summary>

            CmnAjax.Func._ItfToken = token;
        },
        GetSignature: function (json) {
            ...
            for (var _key in json) {
                if (_key != "Cg_sg") {
                    _tmp.push(_key.toLocaleLowerCase());
                    _firstParam.push(_key);
                }
            }
            ...
            for (var _i = 0; _i < _tmp.length; _i++) {
                for (var _j = 0; _j < _firstParam.length; _j++) {
                    if (_firstParam[_j].toUpperCase() == _tmp[_i].toUpperCase()) {
                        break;
                    }
                }

                _paramString += "&" + _tmp[_i] + "=" + json[_firstParam[_j]];
            }

            _paramString = $.md5(_paramString.substring(1, _paramString.length));
        ...
        AddSignature: function (url, param) {
            ...
            if (Cmn.IsType(param, "object")) {
                if (param["Cg_tk"] != undefined || CmnAjax.Func._ItfToken != "") { //存在token说明需要加密
                    if (param["Cg_tk"] == undefined) { param["Cg_tk"] = CmnAjax.Func._ItfToken; }

                    param["Cg_rm"] = Cmn.Math.Random(0, 1);
                    param["Cg_tt"] = new Date().getTime();
                    param["Cg_sg"] = CmnAjax.Func.GetSignature(param);
                }
            }
