...
(function (window, undefined) {
    ...
    var _ImageEditing = function (stageSelector, width, height, editCfg) {
        ...
        //解锁用户操作
        _Self.UnLockUserAction = function () {
            _Self.CurActionState = _BeforeActionState;
        }
        ...
        _Self.SetFilterEffect = function (effectName) {
            ...
            var _filterEffect = _FilterEffects[effectName],
                //滤镜应用的图片对象
                _bitmapContainer = _Self.MainContainer.children[0];

            if (Cmn.IsType(_filterEffect, "function") && _bitmapContainer) {
                //去掉已有的滤镜
                _Self.RemoveFilterEffect();
                _filterEffect.call(_bitmapContainer.children[1], _Self);
            }
        ...
        _Self.RemoveFilterEffect = function () {
            ...
            var _bitmapContainer = _Self.MainContainer.children[0];
            if (_bitmapContainer.children[1]) {
                _bitmapContainer.children[1].filters = [];
                _bitmapContainer.children[1].uncache();
            }
        ...
        _Self.AddStickers = function (image, index) {
            ...
            function _imgLoadComplete() {
                ...
                _bitmap.regX = _image.width / 2;
                _bitmap.regY = _image.height / 2;
                _bitmap.x = _bitmap.regX;
                _bitmap.y = _bitmap.regY;


                _stickers.x = (_Self.Width - _image.width) / 2;
                _stickers.y = (_Self.Height - _image.height) / 2;
                ...
                _bitmap.addEventListener("mousedown", function (e) {
                    _bitmap.dispatchEvent("mouseover");
                    _CurActionObj = _stickers;
                });
                _bitmap.addEventListener("mouseover", function (e) {
                ...
                //当贴图发生变更的时候触发
                _stickers.addEventListener("resize", function (e) {
                    
                    var _x = (_bitmap.image.width * _bitmap.scaleX - _bitmap.image.width) / 2,
                        _y = (_bitmap.image.height * _bitmap.scaleX - _bitmap.image.height) / 2
                    _bgShape.x = _x;
                    _bgShape.y = _y*-1;
                });
        ...
        //获取canvas上面的图片
        _Self.GetImageToBase64 = function () {
            _StickersBulr();
            _Self.Stage.cache(0, 0, _Self.Width, _Self.Height);
            return _Self.Stage.cacheCanvas.toDataURL("image/png");
        }
        ...
        $(_Canvas).on("DOMMouseScroll mousewheel", function (event) {
            ...
            if (_CurActionObj) {
                ...
                _CurActionObj.dispatchEvent("resize");

                if (_CurActionObj.children[1].cacheID > 0) { _CurActionObj.children[1].updateCache(); }
